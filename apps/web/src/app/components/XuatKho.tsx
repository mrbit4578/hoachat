...
const emptyForm = (): Omit<PhieuXuat, 'id'> => ({
  ngayXuat: today(),
...
export function XuatKho() {
  const { hoaChat, phieuXuat, addPhieuXuat, deletePhieuXuat, importPhieuXuat, getFefoSuggestions, getTonKhoByLot, settings } = useChemContext();
  const [search, setSearch] = useState('');
  const [showForm, setShowForm] = useState(false);
  const [form, setForm] = useState<Omit<PhieuXuat, 'id'>>(emptyForm());
  const [deleteId, setDeleteId] = useState<string | null>(null);
  const [filterMonth, setFilterMonth] = useState('');
  const [selectedLot, setSelectedLot] = useState<TonKhoLot | null>(null);

  // FEFO suggestions when a chemical is selected
  const fefoSuggestions = useMemo(() => {
    if (!form.maVatTu) return [];
    return getFefoSuggestions(form.maVatTu);
  }, [form.maVatTu, getFefoSuggestions]);

  const tonKhoByLot = useMemo(() => getTonKhoByLot(), [getTonKhoByLot]);

  // Current lot stock
  const currentLotTon = useMemo(() => {
    if (!form.maVatTu || !form.lotNo) return null;
    return tonKhoByLot.find(t => t.maVatTu === form.maVatTu && t.lotNo === form.lotNo) ?? null;
  }, [form.maVatTu, form.lotNo, tonKhoByLot]);
  ...
  function selectLot(lot: TonKhoLot) {
    setSelectedLot(lot);
    setForm(f => ({ ...f, lotNo: lot.lotNo, hsd: lot.hsd }));
  }
  ...
  function submit() {
    if (!form.maVatTu || !form.lotNo || form.soLuong <= 0) {
      toast.error('Vui lòng chọn hóa chất, lô và nhập số lượng');
      return;
    }
    if (currentLotTon && form.soLuong > currentLotTon.tonLo) {
      toast.error(`Số lượng xuất (${form.soLuong}) vượt quá tồn lô (${currentLotTon.tonLo.toFixed(3)})`);
      return;
    }
    if (currentLotTon?.trangThaiHSD === 'EXPIRED') {
      if (!window.confirm(`Lô ${form.lotNo} đã HẾT HẠN. Bạn có chắc muốn xuất kho không?`)) return;
    }
    addPhieuXuat({ ...form, nguoiXuat: form.nguoiXuat || settings.nguoiLap });
    toast.success(`Đã tạo phiếu xuất ${form.soPhieuXuat}`);
    setForm(emptyForm());
    setSelectedLot(null);
    setShowForm(false);
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <Button size="sm" onClick={() => { setForm(emptyForm()); setSelectedLot(null); setShowForm(true); }}>
          <Plus size={14} /> Tạo phiếu xuất
        </Button>
      </div>

      <div className="flex gap-4 text-sm text-gray-500">
        <span><span className="text-gray-800">{filtered.length}</span> phiếu xuất</span>
            ...
            <div className="p-6 space-y-5">
              ...
              <div className="grid grid-cols-3 gap-4">
                <Field label="Ngày xuất *">
                  <Input type="date" value={form.ngayXuat} onChange={e => setForm(f => ({ ...f, ngayXuat: e.target.value }))} />
                </Field>
                <Field label="Số phiếu xuất">
                  <Input value={form.soPhieuXuat} onChange={e => setForm(f => ({ ...f, soPhieuXuat: e.target.value }))} />
                </Field>
                <Field label="Kho xuất">
                  <select className="w-full h-9 border rounded-md px-2 text-sm" value={form.khoXuat} onChange={e => setForm(f => ({ ...f, khoXuat: e.target.value }))}>
                    {KHO_OPTIONS.map(k => <option key={k}>{k}</option>)}
                  </select>
                </Field>
                <Field label="Bộ phận xuất">
                  <select className="w-full h-9 border rounded-md px-2 text-sm" value={form.boPhanXuat} onChange={e => setForm(f => ({ ...f, boPhanXuat: e.target.value }))}>
                    {BO_PHAN.map(b => <option key={b}>{b}</option>)}
                  </select>
                </Field>
                <Field label="Lý do xuất">
                  <select className="w-full h-9 border rounded-md px-2 text-sm" value={form.lyDo} onChange={e => setForm(f => ({ ...f, lyDo: e.target.value }))}>
                    {LY_DO.map(l => <option key={l}>{l}</option>)}
                  </select>
                </Field>
                <Field label="Người xuất">
                  <Input value={form.nguoiXuat} onChange={e => setForm(f => ({ ...f, nguoiXuat: e.target.value }))} placeholder={settings.nguoiLap} />
                </Field>
              </div>
              ...
              <div className="border border-gray-200 rounded-xl p-4 space-y-3">
                <div className="flex items-center gap-2 text-sm text-gray-600">
                  ...
                  Chọn hóa chất và lô xuất (hệ thống gợi ý FEFO)
                </div>
                <ChemicalQuickSelect
                  ...
                  <div className="space-y-2">
                    ...
                    {fefoSuggestions.length === 0 ? (
                      <div className="text-xs text-gray-400 bg-gray-50 rounded-lg p-3 text-center">
                        Không có lô nào còn tồn kho cho hóa chất này
                      </div>
                    ) : (
                        ...
                        <table className="w-full text-xs">
                          <thead className="bg-gray-50 text-gray-500">
                            <tr>
                              ...
                              <th className="px-3 py-2 text-right">Tồn lô</th>
                          ...
                          <tbody>
                            {fefoSuggestions.map((lot, i) => {
                              ...
                              const isSelected = form.lotNo === lot.lotNo;
                              return (
                                <tr
                                  key={lot.lotNo}
                                ...
                                >
                                  ...
                                  <td className="px-3 py-2 text-right text-gray-800">{lot.tonLo.toFixed(3)} {lot.donViTinh}</td>
                ...
                {/* Quantity and selected lot info */}
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <Field label={`Số lượng xuất (tối đa: ${currentLotTon ? currentLotTon.tonLo.toFixed(3) : '—'} ${form.donViTinh})`}>
                    <Input
                      type="number"
                      min="0"
                      step="0.001"
                      ...
                      className={currentLotTon && form.soLuong > currentLotTon.tonLo ? 'border-red-400' : ''}
                    />
                    {currentLotTon && form.soLuong > currentLotTon.tonLo && (
                      <p className="text-[10px] text-red-500 mt-1">Vượt quá tồn lô!</p>
                    )}
                  </Field>
                </div>
                ...
                {currentLotTon && (
                  <div className={`flex items-center gap-2 px-3 py-2 rounded-lg text-xs ${
                    ...
                    <span>
                      Lô <strong>{currentLotTon.lotNo}</strong> — HSD: {new Date(currentLotTon.hsd).toLocaleDateString('vi-VN')} — Tồn: {currentLotTon.tonLo.toFixed(3)} {currentLotTon.donViTinh} — {currentLotTon.trangThaiHSD}
              ...
              <div className="grid grid-cols-3 gap-4">
                <Field label="Đơn hàng / Mã hàng">
                  <Input value={form.donHangMaHang} onChange={e => setForm(f => ({ ...f, donHangMaHang: e.target.value }))} placeholder="FLEXOSLIP" />
                </Field>
                <Field label="Mã màu">
                  <Input value={form.maMau} onChange={e => setForm(f => ({ ...f, maMau: e.target.value }))} placeholder="LSX-1219" />
                </Field>
                <Field label="Lệnh sản xuất">
                  <Input value={form.lenhSX} onChange={e => setForm(f => ({ ...f, lenhSX: e.target.value }))} />
                </Field>
              </div>
              <Field label="Ghi chú">
      ...
      {deleteId && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 max-w-sm w-full shadow-xl">
            <h3 className="text-gray-800 mb-2">Xác nhận xóa phiếu xuất</h3>
            <p className="text-sm text-gray-500 mb-4">Thao tác này sẽ xóa phiếu xuất và ảnh hưởng đến tồn kho.</p>
            <div className="flex justify-end gap-3">
              <Button variant="outline" size="sm" onClick={() => setDeleteId(null)}>Hủy</Button>
              <Button variant="destructive" size="sm" onClick={() => { deletePhieuXuat(deleteId); setDeleteId(null); toast.success('Đã xóa phiếu xuất'); }}>Xóa</Button>
            </div>
          </div>
        </div>
      )}
